...
function runScript(scriptName, args = []) {
  return new Promise((resolve, reject) => {
    ...
    const child = spawn("node", [path.join(__dirname, scriptName), ...finalArgs], { shell: false, stdio: "inherit" });
...
`);
  ...
  try {
    ...
    if (email) {
      ...
      const tempCartPath = path.join(__dirname, "..", "data", `cart_response_${safeEmail}.json`);
