...
if (fs.existsSync(configPath)) {
  try {
    ...
    if (dbConfig.connectionTimeoutMillis === undefined) {
...
async function getUser(userId) {
  try {
    ...
    console.warn("⚠️ Database query failed (getUser), trying local JSON fallback:", err.message);
...
async function getUserByEmail(email) {
  try {
    ...
    console.warn("⚠️ Database query failed (getUserByEmail), trying local JSON fallback:", err.message);
    const usersJsonPath = path.join(__dirname, '..', 'db_export', 'users.json');
    if (fs.existsSync(usersJsonPath)) {
      try {
        const users = JSON.parse(fs.readFileSync(usersJsonPath, 'utf8'));
...
async function getUserByEmailOrAccountEmail(email) {
  try {
    ...
    console.warn("⚠️ Database query failed (getUserByEmailOrAccountEmail), trying local JSON fallback:", err.message);
    const userDirect = await getUserByEmail(email);
    if (userDirect) return userDirect;
    ...
    if (fs.existsSync(accountsJsonPath)) {
      try {
        ...
        if (acc) {
          return await getUser(acc.user_id);
        }
      } catch (_) {}
    }
  }
...
async function addAccount(userId, email) {
  try {
    ...
    console.warn("⚠️ Database query failed (addAccount), updating local JSON fallback instead:", err.message);
    const tblPath = path.join(__dirname, '..', 'db_export', 'accounts.json');
    if (fs.existsSync(tblPath)) {
      try {
        const rows = JSON.parse(fs.readFileSync(tblPath, 'utf8'));
        const exists = rows.some(r => r.user_id === userId && r.email && r.email.toLowerCase() === email.toLowerCase());
        if (!exists) {
          rows.push({
            user_id: userId,
            email: email,
          ...
          fs.writeFileSync(tblPath, JSON.stringify(rows, null, 2), 'utf8');
          console.log(`✅ [Local Fallback] Added account ${email} locally.`);
        }
      } catch (localErr) {
        console.error("❌ Failed to perform local fallback for addAccount:", localErr.message);
      }
    }
  }
}
...
async function getAccountsByUser(userId) {
  try {
    ...
    console.warn("⚠️ Database query failed (getAccountsByUser), trying local JSON fallback:", err.message);
    const accountsJsonPath = path.join(__dirname, '..', 'db_export', 'accounts.json');
    if (fs.existsSync(accountsJsonPath)) {
      try {
        const accounts = JSON.parse(fs.readFileSync(accountsJsonPath, 'utf8'));
        return accounts.filter(a => a.user_id === userId).map(a => a.email);
      } catch (_) {}
    }
    return [];
  }
...
async function getCookies(userId, email) {
  try {
    ...
    console.warn("⚠️ Database query failed (getCookies), trying local JSON fallback:", err.message);
    const tables = ['accounts', 'success_accounts', 'no_cod_accounts', 'past_order', 'delivery_issue', 'purchase_limit'];
    for (const tbl of tables) {
      const tblPath = path.join(__dirname, '..', 'db_export', `${tbl}.json`);
      if (fs.existsSync(tblPath)) {
        try {
          const rows = JSON.parse(fs.readFileSync(tblPath, 'utf8'));
          const found = rows.find(r => r.user_id === userId && r.email && r.email.toLowerCase() === email.toLowerCase());
          if (found && found.cookies) {
            return typeof found.cookies === 'string' ? JSON.parse(found.cookies) : found.cookies;
          }
        } catch (_) {}
      }
    }
...
async function updateCookies(userId, email, cookies) {
  try {
    ...
    console.warn("⚠️ Database query failed (updateCookies), updating local JSON files instead:", err.message);
    const tables = ['accounts', 'success_accounts', 'no_cod_accounts', 'past_order', 'delivery_issue', 'purchase_limit'];
    let updated = false;
    for (const tbl of tables) {
      const tblPath = path.join(__dirname, '..', 'db_export', `${tbl}.json`);
      if (fs.existsSync(tblPath)) {
        try {
          const rows = JSON.parse(fs.readFileSync(tblPath, 'utf8'));
          const idx = rows.findIndex(r => r.user_id === userId && r.email && r.email.toLowerCase() === email.toLowerCase());
          if (idx !== -1) {
            rows[idx].cookies = cookies;
...
async function moveAccount(email, userId, fromTable, toTable, extraData = {}) {
  ...
  try {
    ...
    console.warn(`⚠️ Database query failed (moveAccount: ${fromTable} -> ${toTable}), trying local JSON fallback:`, e.message);
    try {
      let resolvedUserId = userId;
      let cookies = null;
      ...
      for (const tbl of allowedTables) {
        ...
        if (fs.existsSync(tblPath)) {
          try {
            const rows = JSON.parse(fs.readFileSync(tblPath, 'utf8'));
            const found = rows.find(r => r.email && r.email.toLowerCase() === email.toLowerCase());
            if (found) {
              resolvedUserId = resolvedUserId || found.user_id;
              cookies = cookies || found.cookies;
              updatedAt = found.updated_at || updatedAt;
            }
          } catch (_) {}
        }
      }
      ...
      if (resolvedUserId) {
        // 1. Delete from source table local file
        const fromPath = path.join(__dirname, '..', 'db_export', `${fromTable}.json`);
        if (fs.existsSync(fromPath)) {
          try {
            let rows = JSON.parse(fs.readFileSync(fromPath, 'utf8'));
            rows = rows.filter(r => !(r.user_id === resolvedUserId && r.email && r.email.toLowerCase() === email.toLowerCase()));
            fs.writeFileSync(fromPath, JSON.stringify(rows, null, 2), 'utf8');
          } catch (_) {}
        }

        // 2. Add to destination table local file
        const toPath = path.join(__dirname, '..', 'db_export', `${toTable}.json`);
        if (fs.existsSync(toPath)) {
          try {
            const rows = JSON.parse(fs.readFileSync(toPath, 'utf8'));
            const idx = rows.findIndex(r => r.user_id === resolvedUserId && r.email && r.email.toLowerCase() === email.toLowerCase());
            const newRow = {
              ...
              reason_text: extraData.reason_text || null
            };
            if (idx !== -1) {
            ...
            fs.writeFileSync(toPath, JSON.stringify(rows, null, 2), 'utf8');
            console.log(`✅ [Local Fallback] Moved ${email} from ${fromTable} to ${toTable}`);
          } catch (_) {}
        }
      }
    } catch (localErr) {
      console.error("❌ Failed to perform local fallback for moveAccount:", localErr.message);
    }
  } finally {
...
async function getAccountsFromTable(userId, tableName) {
  ...
  try {
    ...
    console.warn(`⚠️ Database query failed (getAccountsFromTable for ${tableName}), trying local JSON fallback:`, err.message);
    const tblPath = path.join(__dirname, '..', 'db_export', `${tableName}.json`);
    if (fs.existsSync(tblPath)) {
      try {
        const rows = JSON.parse(fs.readFileSync(tblPath, 'utf8'));
