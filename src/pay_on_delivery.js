...
// Helper to extract the actual Amazon order ID from the confirmation URL/HTML, avoiding the session ID.
function extractOrderIdFromPage(url, htmlContent) {
  // 1. Try to extract from URL query parameters (purchaseId or orderId)
  if (url) {
    const purchaseIdMatch = url.match(/[?&](?:purchaseId|orderId)=(\d{3}-\d{7}-\d{7})\b/i);
    if (purchaseIdMatch) {
      return purchaseIdMatch[1];
    }
  }

  // Find the session ID to avoid it
  let sessionId = null;
  if (url) {
    const sessMatch = url.match(/[?&]session-?id=(\d{3}-\d{7}-\d{7})\b/i);
    if (sessMatch) sessionId = sessMatch[1];
  }
  if (!sessionId && htmlContent) {
    const sessMatch = htmlContent.match(/session-?id["'\s:=/]*(\d{3}-\d{7}-\d{7})\b/i);
    if (sessMatch) sessionId = sessMatch[1];
  }

  // 2. Look for any order-ID-like patterns in the URL that are NOT the session ID
  if (url) {
    const matches = url.match(/\b\d{3}-\d{7}-\d{7}\b/g) || [];
    for (const m of matches) {
      if (m !== sessionId) return m;
    }
  }

  // 3. Look in HTML content, avoiding the session ID
  if (htmlContent) {
    const matches = htmlContent.match(/\b\d{3}-\d{7}-\d{7}\b/g) || [];
    for (const m of matches) {
      if (m !== sessionId) return m;
    }
    // If we only found one match, or all matches were excluded, return the first one
...
// Helper to verify if the checkout confirmation page is actually loaded, avoiding false positives in inline scripts
function isOrderPlacedConfirmation(url, htmlContent) {
  if (url && /thankyou|thank-you|order-placed|orderPlaced/i.test(url) && !/sessionID=/i.test(url)) {
    return true;
  }
  if (htmlContent) {
    // Strip scripts and styles to check only user-visible text
    const cleanText = htmlContent
      .replace(/<script[^>]*>[\s\S]*?<\/script>/gi, '')
      .replace(/<style[^>]*>[\s\S]*?<\/style>/gi, '')
      .replace(/<[^>]*>/g, ' ');
    
    if (/thank\s*you|order\s*placed\s*successfully|placed\s*successfully/i.test(cleanText)) {
      return true;
    }
  }
  return false;
...
async function runBrowserFallbackCheckout(email, confirm, checkoutUrl, minPrice, maxPrice, headless = false) {
  ...
  try {
    ...
    if (confirm) {
      ...
      if (isOrderPlacedConfirmation(finalUrl, pageContent)) {
        console.log("\x1b[32m\n🎉 === ORDER PLACED SUCCESSFULLY WITH COD VIA BROWSER FALLBACK! ===\x1b[0m\n");
        const orderId = extractOrderIdFromPage(finalUrl, pageContent);
        ...
        try {
          ...
          if (isSpcPage) {
            ...
            if (confirm) {
              ...
              if (isOrderPlacedConfirmation(finalUrl, finalContent)) {
                console.log("\x1b[32m\n🎉 === ORDER PLACED SUCCESSFULLY VIA BROWSER CHECKOUT! ===\x1b[0m\n");
                orderId = extractOrderIdFromPage(finalUrl, finalContent);
                completed = true;
                break;
              } else {
...
async function runPayOnDeliveryCheckout(email, confirm, headless = false) {
  ...
  if (!initForm) {
    fs.writeFileSync(path.join(__dirname, "..", "debug", "debug_cart.html"), cartHtml, "utf8");
  ...
  if (isOnAddressPage) {
    ...
    fs.writeFileSync(path.join(__dirname, "..", "debug", "debug_address_page.html"), pageHtml, "utf8");
    ...
    if (af) {
      ...
      try {
        ...
        if (postErr.response) {
          fs.writeFileSync(path.join(__dirname, "..", "debug", "debug_address_post_response.html"), typeof postErr.response.data === "string" ? postErr.response.data : JSON.stringify(postErr.response.data), "utf8");
  ...
  for (let attempt = 0; attempt < 3; attempt++) {
    ...
    fs.writeFileSync(path.join(__dirname, "..", "debug", "debug_checkout_stage.html"), pageHtml, "utf8");
    ...
    if (onPay) {
      fs.writeFileSync(path.join(__dirname, "..", "debug", "debug_payment_page.html"), pageHtml, "utf8");
      ...
      if (!pf) {
        fs.writeFileSync(path.join(__dirname, "..", "debug", "debug_pay_noform.html"), pageHtml, "utf8");
      ...
      fs.writeFileSync(path.join(__dirname, "..", "debug", "debug_payment_submit_response.html"), typeof pr.data === "string" ? pr.data : JSON.stringify(pr.data), "utf8");
    ...
    if (onSpc) {
      ...
      if (!pof) { 
        fs.writeFileSync(path.join(__dirname, "..", "debug", "checkout_ready_to_place_order.html"), pageHtml,"utf8"); 
      ...
      if (isOrderPlacedConfirmation(currentUrl, pageHtml)) {
        console.log("\x1b[32m\n🎉 === ORDER PLACED SUCCESSFULLY WITH COD! ===\x1b[0m\n");
        const orderId = extractOrderIdFromPage(currentUrl, pageHtml);
        return { success: true, orderId, orderTotal: finalOrderTotal };
      } else { 
        fs.writeFileSync(path.join(__dirname, "..", "debug", "checkout_unexpected.html"), pageHtml,"utf8"); 
