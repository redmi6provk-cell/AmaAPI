...
function parseArgs() {
  ...
  const config = {
    ...
    help: false,
    // ── IP Rotation ──
    rotateEvery: 3,        // Rotate IP after every N accounts
    waitOnSec: 4,          // Seconds airplane stays ON
    waitReconnectSec: 30,  // Seconds to wait for 4G reconnect
    skipIpRotation: false, // Skip IP rotation entirely
  };
  ...
  for (let i = 0; i < args.length; i++) {
    switch (args[i]) {
      ...
      case "--password":
        ...
        break;
      case "--rotate-every":
        config.rotateEvery = parseInt(args[++i], 10) || 3;
        break;
      case "--wait-on":
      ...
      case "--wait-reconnect":
        ...
        break;
      case "--skip-ip-rotation":
        config.skipIpRotation = true;
        break;
    }
  }
  return config;
...
IP Rotation Options (requires Android phone via USB + ADB):
  --rotate-every <N>       Rotate IP after every N accounts (default: 3)
  --wait-on <N>            Seconds to keep Airplane ON (default: 4)
  --wait-reconnect <N>     Seconds to wait for 4G reconnect after OFF (default: 30)
  --skip-ip-rotation       Disable IP rotation entirely
...
Examples:
  ...
  node get_cookies.js --rotate-every 5              Rotate IP every 5 accounts
  node get_cookies.js --skip-ip-rotation            No IP rotation
`);
}
...
async function main() {
  ...
  // ==================== IP ROTATION SETUP ====================
  let ipRotationEnabled = false;
  let currentIp = null;

  if (!config.skipIpRotation && !config.email) {
    // Only enable IP rotation when processing multiple accounts
    try {
      console.log("\n🔍 Checking ADB connection for IP rotation...");
      const adbStatus = await adb.checkAdbStatus();
      if (adbStatus.adbFound && adbStatus.deviceConnected) {
        ipRotationEnabled = true;
        console.log(`✅ ADB connected: ${adbStatus.deviceId} — IP rotation ENABLED (every ${config.rotateEvery} accounts)`);
        try {
          currentIp = await adb.getPublicIp();
          console.log(`🌐 Current IP: ${currentIp}`);
        } catch (ipErr) {
          console.warn(`⚠️  Could not get current IP: ${ipErr.message}`);
        }
      } else {
        console.log(`⚠️  ADB not available (adbFound=${adbStatus.adbFound}, deviceConnected=${adbStatus.deviceConnected}) — IP rotation DISABLED`);
      }
    } catch (adbErr) {
      console.log(`⚠️  ADB check failed: ${adbErr.message} — IP rotation DISABLED`);
    }
  } else if (config.skipIpRotation) {
    console.log("⏭️  IP rotation skipped (--skip-ip-rotation)");
  }
  ...
  for (let i = 0; i < emailsToProcess.length; i++) {
    ...
    // ── IP Rotation: rotate before processing (except first account) ──
    if (ipRotationEnabled && i > 0 && i % config.rotateEvery === 0) {
      console.log(`\n✈️  [IP Rotation] ${i} accounts done — rotating IP now (every ${config.rotateEvery})...`);
      try {
        ...
        if (rotResult.newIp) {
          ...
          console.log(`✅ [IP Rotation] New IP: ${currentIp}`);
        } else {
          console.warn(`⚠️  [IP Rotation] IP may not have changed. Continuing...`);
        }
      } catch (rotErr) {
        console.warn(`⚠️  [IP Rotation] Failed: ${rotErr.message} — continuing without rotation`);
      }
    }
